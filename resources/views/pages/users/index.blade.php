...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="font-semibold text-lg text-gray-800">Daftar Pengguna</h2>
            <a href="{{ route('users.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md text-sm flex items-center shadow-sm hover:bg-blue-700">
        ...
        <div class="p-6">
            ...
            <div class="mb-6">
                <form action="{{ route('users.index') }}" method="GET">
                    <div class="flex">
                        <input type="text" name="search" placeholder="Cari username, email, nama..." value="{{ request('search') }}"
                    ...
                    <tbody class="text-gray-600 text-sm">
                        @forelse($users as $user)
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                <td class="py-3 px-6 text-left">{{ $user->id_pengguna }}</td>
                                <td class="py-3 px-6 text-left">{{ $user->nama_lengkap }}</td>
                                <td class="py-3 px-6 text-left">{{ $user->email }}</td>
                                <td class="py-3 px-6 text-left">{{ $user->nama_pengguna }}</td>
                                <td class="py-3 px-6 text-left">
                                    @php
                                        ...
                                        if ($user->peran == 'admin') {
                                            ...
                                            $bgColor = 'bg-blue-500';
                                        } elseif ($user->peran == 'tendik') {
                                            $bgColor = 'bg-yellow-500';
                                            $textColor = 'text-gray-800';
                                        } elseif ($user->peran == 'sarpras') {
                                            $bgColor = 'bg-gray-500';
                                ...
                                <td class="py-3 px-6 text-left">{{ $user->nomor_telepon ?? '-' }}</td>
                                <td class="py-3 px-6 text-left">{{ $user->created_at->format('d/m/Y H:i') }}</td>
                                <td class="py-3 px-6 text-center">
                                    <div class="flex item-center justify-center">
                                        <a href="{{ route('users.edit', $user->id_pengguna) }}" class="w-8 h-8 mr-2 transform hover:text-blue-500 hover:scale-110 flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <form action="{{ route('users.destroy', $user->id_pengguna) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus pengguna ini?')" class="w-8 h-8 transform hover:text-red-500 hover:scale-110 flex items-center justify-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-3 px-6 text-center">Tidak ada data pengguna</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
