...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Daftar Ruang</h1>
                <p class="text-sm text-gray-600 mt-1">Kelola data ruang sistem</p>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <form id="searchForm" method="GET" action="{{ route('ruang.index') }}" class="w-full max-w-md">
                    <div class="relative">
                        <input
                            type="text"
                            name="q"
                            value="{{ request('q') }}"
                            placeholder="Cari nama ruang atau gedung..."
                ...
                <button type="button" onclick="window.location.href='{{ route('ruang.create') }}'" 
            ...
            <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="ruangTableBody" class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <form action="{{ route('ruang.destroy', $ruang->id_ruang) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        ...
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus ruang {{ $ruang->nama_ruang }}?')"
                                                        title="Hapus Ruang">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 text-lg font-medium">Tidak ada data ruang</p>
                                            <p class="text-gray-400 text-sm mt-1">Mulai dengan menambahkan ruang baru</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            @if($ruangs->hasPages())
                <div class="mt-6 flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        @if($ruangs->count() > 0)
                            Menampilkan {{ $ruangs->firstItem() }} sampai {{ $ruangs->lastItem() }} dari {{ $ruangs->total() }} ruang
                        @else
                            Tidak ada data
                        @endif
                    </div>
                    <div>
                        {{ $ruangs->appends(['q' => request('q')])->links() }}
                    </div>
                </div>
            @endif
        </div>
    ...
    if (searchInput && tableBody) {
        searchInput.addEventListener('input', function () {
            ...
            timeout = setTimeout(function () {
                fetch(`{{ route('ruang.index') }}?q=${encodeURIComponent(searchInput.value)}`)
                    .then(response => response.text())
                    .then(html => {
                        // Ambil tbody dari response HTML
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const newTbody = doc.getElementById('ruangTableBody');
                        if (newTbody) {
