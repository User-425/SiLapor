...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Daftar Gedung</h1>
                <p class="text-sm text-gray-600 mt-1">Kelola data gedung sistem</p>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <form id="searchForm" method="GET" action="{{ route('gedung.index') }}" class="w-full max-w-md">
                    <div class="relative">
                        <input
                            type="text"
                            name="q"
                            value="{{ request('q') }}"
                            placeholder="Cari nama gedung atau deskripsi..."
                ...
                <button type="button" onclick="window.location.href='完毕

System: '{{ route('gedung.create') }}'" 
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Gedung</th>
            ...
            <tbody id="gedungTableBody" class="bg-white divide-y divide-gray-200">
                @forelse($gedungs as $index => $gedung)
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $index + $gedungs->firstItem() }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $gedung->nama_gedung }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $gedung->deskripsi_lokasi ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <form action="{{ route('gedung.destroy', $gedung->id_gedung) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            ...
                                            onclick="return confirm('Apakah Anda yakin ingin menghapus gedung {{ $gedung->nama_gedung }}?')"
                                            title="Hapus Gedung">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 text-lg font-medium">Tidak ada data gedung</p>
                                <p class="text-gray-400 text-sm mt-1">Mulai dengan menambahkan gedung baru</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    @if($gedungs->hasPages())
        <div class="mt-6 flex items-center justify-between">
            <div class="text-sm text-gray-700">
                @if($gedungs->count() > 0)
                    Menampilkan {{ $gedungs->firstItem() }} sampai {{ $gedungs->lastItem() }} dari {{ $gedungs->total() }} gedung
                @else
                    Tidak ada data
                @endif
            </div>
            <div>
                {{ $gedungs->appends(['q' => request('q')])->links() }}
            </div>
        </div>
    @endif
</div>
    ...
    if (searchInput && tableBody) {
        searchInput.addEventListener('input', function () {
            ...
            timeout = setTimeout(function () {
                fetch(`{{ route('gedung.index') }}?q=${encodeURIComponent(searchInput.value)}`)
                    .then(response => response.text())
                    .then(html => {
                        // Ambil tbody dari response HTML
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        const newTbody = doc.getElementById('gedungTableBody');
                        if (newTbody) {
