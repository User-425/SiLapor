...
@section('title', 'Edit Fasilitas Ruang')

@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Fasilitas Ruang</h1>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-6">
            ...
            <form action="{{ route('fasilitas.update', $fasRuang->id_fas_ruang) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select id="id_ruang" name="id_ruang" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach($ruangs as $ruang)
                            <option value="{{ $ruang->id_ruang }}" {{ old('id_ruang', $fasRuang->id_ruang) == $ruang->id_ruang ? 'selected' : '' }}>
                                {{ $ruang->nama_ruang }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select id="id_fasilitas" name="id_fasilitas" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        ...
                        @foreach($fasilitas as $fas)
                            <option value="{{ $fas->id_fasilitas }}" {{ old('id_fasilitas', $fasRuang->id_fasilitas) == $fas->id_fasilitas ? 'selected' : '' }}>
                                {{ $fas->nama_fasilitas }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="kode_fasilitas" name="kode_fasilitas" 
                           value="{{ old('kode_fasilitas', $fasRuang->kode_fasilitas) }}" 
