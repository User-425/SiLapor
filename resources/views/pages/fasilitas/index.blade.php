...
@section('title', 'Manajemen Unit Fasilitas')
...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Daftar Unit Fasilitas</h1>
                <p class="text-sm text-gray-600 mt-1">Kelola data unit fasilitas sistem</p>
            </div>
        </div>
        ...
        <div class="p-6">
                ...
                <form id="searchForm" method="GET" action="{{ route('fasilitas.index') }}" class="w-full max-w-md">
                    <div class="relative">
                        <input
                            ...
                            name="q"
                            value="{{ request('q') }}"
                            placeholder="Cari kode, fasilitas, atau ruang..."
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="fasilitasTableBody" class="bg-white divide-y divide-gray-200">
                            @forelse($fasRuangs as $index => $fasRuang)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $index + $fasRuangs->firstItem() }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fasRuang->kode_fasilitas }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fasRuang->fasilitas->nama_fasilitas }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fasRuang->ruang->nama_ruang }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <a href="{{ route('fasilitas.history', $fasRuang->id_fas_ruang) }}" 
                                               class="inline-flex items-center p-2 text-purple-600 hover:text-purple-800 hover:bg-purple-50 rounded-lg transition-all duration-200" 
                                               title="Riwayat Kerusakan">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('fasilitas.destroy', $fasRuang->id_fas_ruang) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        ...
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus unit fasilitas {{ $fasRuang->kode_fasilitas }}?')"
                                                        title="Hapus Unit Fasilitas">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 text-lg font-medium">Tidak ada data unit fasilitas</p>
                                            <p class="text-gray-400 text-sm mt-1">Mulai dengan menambahkan unit fasilitas baru</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <!-- Pagination -->
            @if($fasRuangs->hasPages())
                <div class="mt-6 flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        @if($fasRuangs->count() > 0)
                            Menampilkan {{ $fasRuangs->firstItem() }} sampai {{ $fasRuangs->lastItem() }} dari {{ $fasRuangs->total() }} unit fasilitas
                        @else
                            Tidak ada data
                        @endif
                    </div>
                    <div>
                        {{ $fasRuangs->appends(['q' => request('q')])->links() }}
                    </div>
                </div>
            @endif
        </div>
